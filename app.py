...
BOT_TOKEN = "7980723104:AAHU5aDxkzLcIobRGQfrz4CWpCxnV07XAXA"
...
def webhook():
    """معالجة رسائل تليجرام عبر webhook"""
    try:
...
⏰ سيعود للتوفر بعد {account_manager.db['settings']['cooldown_hours']} ساعة
""")
                        logger.info(f"✅ Account sent to user {chat_id}: {email}")
                    else:
                        stats = account_manager.get_statistics()
                        message = "❌ <b>مفيش حسابات متاحة دلوقتي</b>\n\n"

                        if stats.get("next_available"):
                            message += f"⏰ <b>أقرب حساب متاح:</b>\n"
                            message += f"📧 {stats['next_available_email']}\n"
                            message += f"🕒 بعد: {stats['next_available']}\n\n"
                        ...
                        logger.warning(f"⚠️ No accounts available for user {chat_id}")
        ...
        # معالجة الأزرار
            ...
            answer_callback(callback_query_id, "✅ تم")

            # معالجة الأزرار
            if callback_data == 'get_account':
...
👤 اسم المستخدم: <code>{account['username']}</code>
🔑 كلمة المرور: <code>{account['password']}</code>

⏰ سيعود للتوفر بعد 24 ساعة
""")
                else:
                    send_telegram_text(chat_id, "❌ لا توجد حسابات متاحة حالياً")
...
def answer_callback(callback_query_id, text=None):
    ...
    try:
        ...
        logger.error(f"❌ Answer callback error: {str(e)}")
        return None
    ...
    try:
        ...
        <body style="padding:20px; font-family:Arial;">
            ...
            <div style="margin-top:20px;">
                <a href="/webhook_info">📊 عرض معلومات Webhook</a> |
                <a href="/">🏠 الصفحة الرئيسية</a>
            </div>
            <div style="margin-top:20px; padding:10px; background:#d4edda; border-radius:5px;">
                <p>✅ <strong>لو النتيجة "ok": true</strong> يبقى البوت شغال دلوقتي!</p>
                <p>💬 جرب تبعت أي رسالة للبوت على تليجرام</p>
            </div>
        </body>
        </html>
        """
...
def webhook_info():
    ...
    try:
        ...
        <body style="padding:20px; font-family:Arial;">
            <h2>📊 معلومات Webhook</h2>
            <pre style="background:#f5f5f5; padding:15px; border-radius:5px;">{json.dumps(info, indent=2, ensure_ascii=False)}</pre>
            <div style="margin-top:20px;">
                <a href="/set_webhook">🔄 إعادة تعيين Webhook</a> |
                <a href="/">🏠 الصفحة الرئيسية</a>
            </div>
        </body>
        </html>
...
def home():
    ...
    return f"""
    <html>
    <head><title>Telegram Bot Status</title><meta charset="UTF-8"></head>
    <body style="font-family: Arial; padding: 20px; background: #f5f5f5;">
        <h1>✅ البوت شغال!</h1>
        <div style="background: white; padding: 15px; border-radius: 8px; margin-top: 20px;">
            <h2>📊 الإحصائيات:</h2>
...
def set_webhook():
    ...
    try:
        ...
        <body style="padding:20px; font-family:Arial;">
            ...
            <div style="margin-top:20px; padding:10px; background:#d4edda; border-radius:5px;">
                <p>✅ <strong>لو النتيجة "ok": true</strong> يبقى البوت شغال دلوقتي!</p>
                <p>💬 جرب تبعت رسالة للبوت على تليجرام</p>
            </div>
        </body>
        </html>
        """
...
def webhook_info():
    ...
    try:
        ...
        return f"""
        <html>
        <head><title>Webhook Info</title><meta charset="UTF-8"></head>
        <body style="padding:20px; font-family:Arial;">
            <h2>📊 معلومات Webhook</h2>
            <pre style="background:#f5f5f5; padding:15px; border-radius:5px;">{json.dumps(info, indent=2, ensure_ascii=False)}</pre>
            <div style="margin-top:20px;">
