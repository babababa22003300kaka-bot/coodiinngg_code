...
def parse_sender_data(text: str) -> Dict:
    ...
    # 🧠 الكلمات الذكية للسحب والإبقاء - نسخة محسّنة وأكثر ذكاءً
    TAKE_KEYWORDS = [
        # الصيغ الأساسية
        "اسحب", "اسحبي", "اسحبو", "اسحبوا", "اسحبوووا",
        "يسحب", "يسحبو", "يسحبوا",
        "سحب", "سحبو", "سحبوا",
        # الأخطاء الإملائية الشائعة
        "اصحب", "اصحبي", "اصحبو", "اصحبوا",
        "سحبت", "اسحبت", 
        # صيغ إضافية
        "خد", "خذ", "خدي", "خدو", "خدوا",
        "هات", "هاتي", "هاتو", "هاتوا",
        ...
        "شيل", "شيلي", "شيلو", "شيلوا",
        # English
        "take", "get", "pull", "withdraw",
    ]

    KEEP_KEYWORDS = [
        # الصيغ الأساسية
        "يسيب", "سيب", "سيبي", "سيبو", "سيبوا", "سيبوووا",
        "اسيب", "اسيبي", "اسيبو", "اسيبوا",
        "سيبت", "اسيبت",
        # الأخطاء الإملائية الشائعة
        "صيب", "صيبي", "صيبو", "صيبوا",
        "اصيب", "اصيبي", "اصيبو", "اصيبوا",
        # صيغ إضافية
        "خلي", "خليي", "خليو", "خليوا",
        "ابقي", "ابقى", "ابقو", "ابقوا",
        "خلى", "خلو", "خلوا",
        "اترك", "اتركي", "اتركو", "اتركوا",
        "سب", "سبي", "سبو", "سبوا",
        "حط", "حطي", "حطو", "حطوا",
        # English
        "keep", "leave", "hold", "remain",
    ]
...
def extract_amount_smart(line: str, keywords: List[str]) -> str:
    """
    🧠 استخلاص ذكي ومرن للمبلغ من السطر
    
    يدعم:
    - الأرقام الملتصقة: اسحب200، سيب50
    - مع مسافات: اسحب 200، سيب 50
    - مع فواصل وعلامات: اسحب: 200، سيب - 50
    - أخطاء إملائية: اسحبووو200
    - حروف مكررة: سيييب50
    """
    ...
    for keyword in keywords:
        # إنشاء نسخة من الكلمة تتحمل الحروف المكررة
        # مثلاً: "سيب" تصبح "س+ي+ب+" عشان تقبل "سيييب"
        flexible_keyword = '+'.join([c for c in keyword]) + '+'
        
        # Pattern 1: الحروف المكررة + الرقم (مثل: سيييب50 أو اسحبببب200)
        pattern1 = rf"{flexible_keyword}\s*(\d+)"
        match = re.search(pattern1, line_normalized)
        if match:
            return match.group(1)
        
        # Pattern 2: الكلمة العادية + الرقم (مثل: اسحب200 أو سيب 50)
        pattern2 = rf"{keyword}\s*(\d+)"
        match = re.search(pattern2, line_normalized)
        if match:
            return match.group(1)
        
        # Pattern 3: مع علامات الترقيم (مثل: اسحب: 200 أو سيب - 50)
        pattern3 = rf"{keyword}\s*[:\-_,،؛;\.\s]+\s*(\d+)"
        match = re.search(pattern3, line_normalized)
        if match:
            return match.group(1)
